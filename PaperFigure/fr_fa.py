...
def plot_impedance_with_markers(f, zmag, phase, fr, fa, table):
    f = np.asarray(f, dtype=float)
    zmag = np.asarray(zmag, dtype=float)
    phase = np.asarray(phase, dtype=float)

    fig, (ax1, ax2) = plt.subplots(2, 1, sharex=True, figsize=(10, 6))
    ax1.semilogx(f, zmag, "k-", linewidth=1.8, label="|Z|")
    ax2.semilogx(f, phase, "k-", linewidth=1.8, label="Phase")

    if np.isfinite(fr):
        ax1.axvline(fr, color="tab:red", linestyle="--", linewidth=1.5, label=f"fr = {fr:.4g} Hz")
        ax2.axvline(fr, color="tab:red", linestyle="--", linewidth=1.5, label=f"fr = {fr:.4g} Hz")
    if np.isfinite(fa):
        ax1.axvline(fa, color="tab:blue", linestyle="--", linewidth=1.5, label=f"fa = {fa:.4g} Hz")
        ax2.axvline(fa, color="tab:blue", linestyle="--", linewidth=1.5, label=f"fa = {fa:.4g} Hz")
    ...
    fig.suptitle(f"{table} |Z| & Phase with fr/fa markers")
