...
def main():
    ...
    full_mask = exp["Freq"].to_numpy(dtype=float) >= FULL_FMIN
    exp = exp.loc[full_mask].copy()
    ...
    # ---- restrict data used for fitting only ----
    fit_mask = (f_all >= FIT_FMIN) & (f_all <= FIT_FMAX)
    f_fit_all = f_all[fit_mask]
    zabs_fit_all = zabs_all[fit_mask]
    phase_fit_all = phase_all[fit_mask]
    if f_fit_all.size == 0:
        raise ValueError(f"No fitting data in [{FIT_FMIN}, {FIT_FMAX}] Hz.")

    # ---- sample for fitting ----
    f_fit, zabs_fit, phase_fit = sample_freq_points(
        f_fit_all, zabs_fit_all, phase_fit_all,
        n_samples=N_SAMPLES,
        mode=SAMPLE_MODE,
        seed=SEED,
    )
    ...
    # Use log-spaced points across the full loaded experiment range
