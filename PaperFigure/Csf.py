...
def plot_z_phase_with_windows(f, mag, th_deg, table, sL=None, sH=None, title_suffix=""):
    ...
    ax1.semilogx(f, mag, "k-", linewidth=1.8)
    ax1.set_ylabel("|Z| (Ω)")
    ax1.grid(True, which="both", linestyle="--", linewidth=0.5)

    ax2.semilogx(f, th_deg, "k-", linewidth=1.8)
    ...
    if sL is not None:
        ...
        f_lo, f_hi = float(f[i0]), float(f[i1])
        ax1.axvspan(f_lo, f_hi, color=lf_color, alpha=0.12)
        ax2.axvspan(f_lo, f_hi, color=lf_color, alpha=0.12)
        ax1.axvline(f_lo, color=lf_color, linestyle="--", linewidth=1.0)
        ax1.axvline(f_hi, color=lf_color, linestyle="--", linewidth=1.0)
        ax2.axvline(f_lo, color=lf_color, linestyle="--", linewidth=1.0)
        ax2.axvline(f_hi, color=lf_color, linestyle="--", linewidth=1.0)
        ax2.text(f_lo, ax2.get_ylim()[1] * 0.85, "LF window", color=lf_color, fontsize=10, va="top")

    if sH is not None:
        i0, i1 = sH.start, sH.stop - 1
        f_lo, f_hi = float(f[i0]), float(f[i1])
        ax1.axvspan(f_lo, f_hi, color=hf_color, alpha=0.12)
        ax2.axvspan(f_lo, f_hi, color=hf_color, alpha=0.12)
        ax1.axvline(f_lo, color=hf_color, linestyle="--", linewidth=1.0)
        ax1.axvline(f_hi, color=hf_color, linestyle="--", linewidth=1.0)
        ax2.axvline(f_lo, color=hf_color, linestyle="--", linewidth=1.0)
        ax2.axvline(f_hi, color=hf_color, linestyle="--", linewidth=1.0)
        ax2.text(f_lo, ax2.get_ylim()[1] * 0.70, "HF window", color=hf_color, fontsize=10, va="top")
